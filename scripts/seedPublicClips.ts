// Sample public clips seeder script
// Run this in browser console while on the app or via node

import { collection, addDoc } from 'firebase/firestore';
import { db } from './lib/firebase';

const samplePublicClips = [
    {
...
export async function seedPublicClips() {
    console.log('Seeding public clips...');

    for (const clip of samplePublicClips) {
        try {
            const docRef = await addDoc(collection(db, 'publicClips'), clip);
            console.log('Added clip:', clip.title, '-> ID:', docRef.id);
        } catch (error) {
            console.error('Error adding clip:', clip.title, error);
        }
    }
